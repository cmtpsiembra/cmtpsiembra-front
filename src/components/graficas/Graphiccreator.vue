<template>
  <section class="section">
    <div class="buttons is-centered">
      ...
      <span class="button is-info tooltip" data-tooltip="Cambiar color" @click="ejecuta('color')">
        <span class="icon">
      ...
      <span
        class="button is-info tooltip"
        data-tooltip="Cambiar color felicidad"
        @click="ejecuta('colorfelicidad')"
      >
        <span class="icon">
...
export default {
  ...
  methods: {
    ejecuta(e) {
      ...
      if (e === "colorfelicidad") {
        if (this.controlcolor === 12) {
          this.controlcolor = 16;
        } else {
          this.controlcolor = 12;
        }
        this.convertData();
      }
      this.addGadgets();
      ...
      if (_this.grupos.length > 0 && _this.variables.length > 0) {
        _this.grupos.forEach(function(k1, v1) {
          ...
          _this.variables.forEach(function(k2, v2) {
            // este codigo es para que no muestre valores negativos en las graficas
            let n = k1[k2.cod].sum;
            if (n < 0) {
              n = 0;
            }
  ...
  watch: {
    control(nv, ov) {
      if (this.grupos.length > 0 && this.variables.length > 0) {
        this.convertData();
        this.addGadgets();
      }
    },
