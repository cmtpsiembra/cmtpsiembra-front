<template>
  <div>
    ...
    <section v-if="isCorrect" class="section">
      <div class="container">
        <section class="hero is-light">
          <div class="hero-body">
            <div class="container">
              <h1 class="title">{{datalider.empleado.nombre}}</h1>
              <h2
                class="subtitle"
              >{{datalider.empleado.cargo}} - {{datalider.empleado.dependencia}} - {{datalider.empleado.email}}</h2>
            </div>
          </div>
        </section>
...
export default {
  ...
  methods: {
    async enviar() {
      ...
      let result;
      try {
        if (_this.$route.params.relacionid) {
          const data = {
            relacionid: _this.$route.params.relacionid,
            liderid: _this.$route.params.liderid,
            procesoid: _this.$route.params.procesoid,
            password: _this.password,
          };
          result = await apiServices.post(`/resultliderfree`, data);
          if (result.data) {
    ...
    graficaradar() {
      console.log("ejecuta graficaradar");
      const _this = this;
      this.radar.labels = [];
      this.bar.labels = [];
    ...
    async traeData(_this) {
      console.log("ejecuta traedata");
      let result;
      try {
        if (this.$route.name === "sharelider") {
        } else {
          this.isCorrect = true;
          result = await apiServices.get(
            `/resultlider/${_this.$route.params.procesoid}/${_this.$route.params.liderid}`
          );
          _this.datalider = result.data;
          const campos = Object.keys(_this.cambio);
          campos.forEach(function (k) {
