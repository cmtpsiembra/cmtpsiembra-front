  ...
  <div class="modal" v-bind:class="{ 'is-active': show }">
    ...
    <div class="modal-card">
      <header class="modal-card-head">
        <h1 class="title">Selecciona un evalulador</h1>
...
export default {
  ...
  methods: {
    employeeSelected(employee){
      this.$emit("evaluatorEmployee", employee);
      this.closeModal()
    },
    closeModal() {
      this.$emit("showSelectEvaluator", false);
    },
    next() {
    ...
    async find() {
      ...
      try {
        ...
        console.log("empleados search: ", f.search);
