...
const parcoords = function(config) {
  var __ = {
    data: [],
    highlighted: [],
    dimensions: {},
  ...
  var dragging = {};
  // var line = d3.svg.line(); // TODO
  d3.svg.line(); // TODO
  var axis = d3.svg
    .axis()
    .orient("left")
    .ticks(5);
  var g; // groups for axes, brushes
  var ctx = {};
  var canvas = {};
  // var clusterCentroids = []; // TODO

  // side effects for setters
  var sideEffects = d3.dispatch
    ...
    .on("bundleDimension", function(d) {
      ...
      __.clusterCentroids = computeClusterCentroids(__.bundleDimension);
      if (flags.interactive) {
        pc.render();
      }
    })
  ...
  function getset(obj, state, events) {
    d3.keys(state).forEach(function(key) {
      obj[key] = function(x) {
        ...
        var old = state[key];
        state[key] = x;
        sideEffects[key].call(pc, { value: x, previous: old });
  ...
  pc.commonScale = function(global, type) {
    ...
    //   return __.dimensions[p].type === t; // todo
    // });
  ...
  pc.toTypeCoerceNumbers = function(v) {
    let stringv = parseFloat(v);
    stringv = stringv.toString();
    if (stringv === v && v != null) {
      //  todo
      return "number";
    }
  ...
  pc.render.default = function() {
    ...
    __.data.forEach(pathForeground);
  };

  // var foregroundQueue = d3
  //   .renderQueue(pathForeground)
  //   .rate(50)
  //   .clear(function() {
  //     pc.clear("foreground");
  //     pc.clear("highlight");
  ...
  pc.renderBrushed.default = function() {
    ...
    if (isBrushed()) {
      __.brushed.forEach(pathBrushed);
    }
  };

  // var brushedQueue = d3
  //   .renderQueue(pathBrushed)
  //   .rate(50)
  //   .clear(function() {
  //     pc.clear("brushed");
  //   });
  ...
  //   }
  // };
  function computeClusterCentroids(d) {
  ...
  function computeControlPoints(centroids) {
  ...
  function singleCurve(d, ctx) {
    var centroids = computeCentroids(d);
    var cps = computeControlPoints(centroids);
  ...
  function pathBrushed(d, i) {
    // Aqui puedo ver la data seleccionada
    if (__.brushedColor !== null) {
      ctx.brushed.strokeStyle = d3.functor(__.brushedColor)(d, i);
    } else {
      ctx.brushed.strokeStyle = d3.functor(__.color)(d, i);
    }
    return colorPath(d, ctx.brushed);
  }

  function pathForeground(d, i) {
    ctx.foreground.strokeStyle = d3.functor(__.color)(d, i);
    return colorPath(d, ctx.foreground);
  }

  function pathHighlight(d, i) {
    ctx.highlight.strokeStyle = d3.functor(__.color)(d, i);
    return colorPath(d, ctx.highlight);
  }
  ...
  function flipAxisAndUpdatePCP(dimension) {
    ...
    g = pc.svg.selectAll(".dimension"); // todo
  ...
  pc.brushMode = function(mode) {
    ...
    if (pc.brushModes().indexOf(mode) === -1) {
      // throw "pc.brushmode: Unsupported brush mode: " + mode; // todo
      throw new Error({
        code: "error",
        message: `pc.brushmode: Unsupported brush mode: ${mode}`,
      }); // todo
    }
  ...
  (function() {
    ...
    function isBrushed(p) {
      return !brushes[p].empty();
    }

    // data within extents
    function selected() {
      var actives = d3.keys(__.dimensions).filter(isBrushed);
      ...
      return __.data.filter(function(d) {
        switch (brush.predicate) {
          ...
          default:
            ...
            throw new Error({
              code: "error",
              message: `Unknown brush predicate ${__.brushPredicate}`,
            }); // todo
        }
      });
    }
  ...
  (function() {
    ...
    function drawStrum(strum, activePoint) {
      ...
      var id = strum.dims.i;
      var points = [strum.p1, strum.p2];
      var line = svg.selectAll("line#strum-" + id).data([strum]);
      var circles = svg.selectAll("circle#strum-" + id).data(points);
    ...
    function containmentTest(strum, width) {
      ...
      var p2 = [strum.p2[0] - strum.minX, strum.p2[1] - strum.minX];
      var m1 = 1 - width / p1[0];
      var b1 = p1[1] * (1 - m1);
      var m2 = 1 - width / p2[0];
      var b2 = p2[1] * (1 - m2);
    ...
    function selected() {
      ...
      function crossesStrum(d, id) {
        var strum = strums[id];
        var test = containmentTest(strum, strums.width(id));
        var d1 = strum.dims.left;
        var d2 = strum.dims.right;
        var y1 = __.dimensions[d1].yscale;
        var y2 = __.dimensions[d2].yscale;
  ...
  (function() {
    ...
    function drawStrum(arc, activePoint) {
      ...
      var id = arc.dims.i;
      var points = [arc.p2, arc.p3];
      var line = svg.selectAll("line#arc-" + id).data([
        { p1: arc.p1, p2: arc.p2 },
        { p1: arc.p1, p2: arc.p3 },
      ]);
      var circles = svg.selectAll("circle#arc-" + id).data(points);
      var drag = d3.behavior.drag();
      var path = svg.selectAll("path#arc-" + id).data([arc]);
    ...
    function selected() {
      ...
      function crossesStrum(d, id) {
        var arc = arcs[id];
        var test = containmentTest(arc);
        var d1 = arc.dims.left;
        var d2 = arc.dims.right;
        var y1 = __.dimensions[d1].yscale;
        var y2 = __.dimensions[d2].yscale;
        var a = arcs.width(id);
        var b = y1(d[d1]) - y2(d[d2]);
        var c = hypothenuse(a, b);
        var angle = Math.asin(b / c); // rad in [-PI/2, PI/2]
        return test(angle);
      }
  ...
  pc.mergeParcoords = function(callback) {
    ...
    // var DOMURL = window.URL || window.webkitURL || window; // todo
...
const renderQueue = function(func) {
  ...
  var _rate = 10; // number of calls per frame
  var _clear = function() {}; // clearing function
  var _i = 0; // current iteration
